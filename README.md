# 🌍 Giverly UAE Expansion  
### Designing a Fintech Ecosystem that Turns Everyday Spending into Social Impact
[Project Summary](Notes/Project-Summary.md)
---

## 🚀 What This Project Is About

How can everyday transactions be transformed into meaningful charitable contributions without changing user behaviour?

This project explores the expansion of **Giverly**, a fintech-for-good platform, into the UAE market by designing a solution that integrates:

- Cashback rewards  
- Charitable donations  
- Gamified user engagement  

---

## 🧠 The Core Idea

Instead of asking users to donate more, the platform embeds giving into what they already do.

**Spend → Earn → Give → Feel Impact → Repeat**

---

## 🎯 Business Challenge

Expanding into the UAE requires addressing:

- Cultural and financial expectations, including Islamic finance considerations  
- Low engagement with traditional donation platforms  
- User reluctance to donate without direct personal benefit  
- The need for differentiation in a competitive fintech landscape  

---

## 🔍 My Role

**Business Analyst & Product Owner**

I contributed across the full lifecycle of the solution:

- Structured Agile sprint planning and delivery phases  
- Translated business needs into product and system requirements  
- Created user research artefacts including persona, empathy map, and customer journey  
- Contributed to MVP definition and prototype development  
- Analysed stakeholder feedback and supported iterative refinement  

What I personally contributed?

- Key contributions included sprint planning, Persona 3 development, empathy mapping, customer journey analysis, and prototype contribution for UAE user scenarios.
---

## 👥 User-Centred Design

### Persona: Khalid Al Sharqi (Young Professional)

- Tech-savvy and convenience-driven  
- Interested in financial efficiency and rewards  
- Motivated by transparency and visible impact  

### Persona Assets
- [Persona](Assets/persona-khalid.png)
- [Empathy Map](Assets/empathy-map-khalid.png)
- [Customer Journey Map](Assets/customer-journey-khalid.png)

---

## 💡 Key Insights

- Users prefer passive giving through mechanisms like cashback donations  
- Trust increases when donation flows are transparent and trackable  
- Engagement improves when users receive instant rewards and gamified feedback  
- Solutions must fit existing lifestyle habits rather than require behavioural change  

---

## 🧩 Customer Journey Thinking

Mapped across:

**Awareness → Consideration → Conversion → Retention → Advocacy**

### Pain Points
- Donation platforms feel disconnected from daily life  
- Limited transparency reduces trust  
- No personal benefit discourages repeated engagement  

### Opportunities
- Embed donations into transactions  
- Provide real-time impact tracking  
- Reward participation through gamification and cashback  

---

## ⚙️ From Insight to Solution

### MVP Strategy

Using **MoSCoW prioritisation**, the project focused on:

- High-value and high-impact features  
- Feasible solutions within scope  
- Alignment with user behaviour and business value  

---

## 💡 Proposed Solution

A mobile-first fintech platform connecting:

**Spending → Cashback → Donations → Impact Tracking**

### Core Features
- Cashback automatically linked to donations  
- Gamified engagement through rewards, badges, and progress tracking  
- AI-driven recommendations for causes and giving behaviour  
- Transparent donation tracking dashboard  
- Multilingual and culturally adapted user experience  

---

## 🏗 Platform Design & Wireframes

### 🎥 Demo

Watch the product walkthrough:

  [▶️ Watch Demo Video](https://www.loom.com/share/ef79d2a043a344e49eed8227f0c4c396)

### Architecture
- [Platform Architecture Overview](Assets/architecture-overview.png)

### Screens
- [Welcome and Onboarding](Assets/onboarding.png)
- [Main Dashboard and Impact Tracking](Assets/dashboard.png)
- [Impact Tracking](Assets/impact-tracking.png)
- [Gamified Engagement System](Assets/gamified-engagement.png)
- [AI-Driven Recommendation Engine](Assets/AI-recommendation-engine.png)
- [Transaction Security System](Assets/transaction-processing-1.png)
- [Transaction Processing System](Assets/transaction-processing-2.png)
- [Organisation Management Suite](Assets/organisation-suite.png)
- [Community Engagement Platform](Assets/community-platform.png)
- [Islamic Finance Hub](Assets/islamic-finance-hub.png)
- [Support Infrastructure](Assets/support-infrastructure.png)
- [Animal Welfare Campaign Suite](Assets/animal-welfare-suite.png)

---

## 🔄 Agile Delivery Approach

The project followed a structured Agile and design thinking workflow:

- Phase-based sprint planning  
- Continuous stakeholder collaboration  
- Iterative development and refinement  
- Alignment between business objectives, user needs, and technical feasibility  

---

## 🧪 Testing & Iteration

The prototype was refined through:

- Usability testing  
- Stakeholder validation  
- Feedback analysis  
- Iterative design improvements  

Key refinement areas included:
- Dashboard clarity  
- Navigation flow  
- Privacy and security visibility  
- Educational content for users  

---

## 📈 Outcome

- Delivered a complete business and product solution for UAE market entry  
- Achieved **High Distinction (90.6%)**  
- Recognised for strong analytical depth, critical thinking, and professional-quality recommendations. 

---

## 🧠 Key Learnings

- Translating user behaviour into product strategy  
- Balancing innovation with feasibility and cultural context  
- Connecting business, user, and technology perspectives into one solution  

---

## 🔧 Tools & Methods

- Jira  
- Figma / Sketch  
- Stanford Design Thinking Framework  
- BPMN  
- MoSCoW Prioritisation  
- Agile / Sprint Planning  

---

## 📄 Supporting Documents

- [Final Report](Docs/final-report.pdf)
- [Milestone 1 Report](Docs/milestone-1-report.pdf)
- [Slides 1](Docs/slide-milestone-1.pdf)
- [Milestone 2 Report](Docs/milestone-2-report.pdf)
- [Slides 2](Docs/slide-milestone-2.pdf)

---

## 👤 Author

**Petch Arpornpattana**  
Business Analyst | Product-Oriented Thinker

Focused on solving business problems by combining user insight, product thinking, and structured analysis.
